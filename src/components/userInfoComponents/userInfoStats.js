...
class UserInfoStats extends Component {
  ...
  componentDidMount(){
    fetch("http://localhost:3000/api/v1/reviews")
      .then(res => res.json())
      .then(data => {
        let userBountyIds = this.props.targetUser.bounties.map(b => b.id);
        let userProjectIds = this.props.targetUser.projects.map(p => p.id);
    ...
    return(
      <div style={statsStyle}>
        <Segment>
          ...
          <p>Projects Completed: {(this.props.targetUser.projects).filter(proj => proj.status === "100").length}</p>
        </Segment>
      </div>
    );
  }
